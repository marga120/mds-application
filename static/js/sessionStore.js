...
const SessionStore = {
  ...
  initializeSession: async function () {
    ...
    if (existingId) {
      // Validate the stored session still exists and is not archived
      const isValid = await this._validateSession(existingId);
      if (isValid) {
        return existingId;
      }
      // Stored session was archived or removed — clear and fall through
      this.clearCurrentSession();
    }

    // No valid stored session: fetch the campus default from the API
    try {
  ...
  /**
   * Validate that a stored session ID still exists in the active session list.
   *
   * Fetches all non-archived sessions and checks whether the given ID is
   * present. Returns true on network error to avoid clearing a valid session
   * due to a transient failure.
   *
   * @private
   * @param {number} sessionId - The stored session ID to validate
   * @returns {Promise<boolean>} True if the session is still active
   */
  _validateSession: async function (sessionId) {
    try {
      const response = await fetch("/api/sessions");
      const result = await response.json();
      if (!result.success || !result.sessions) return false;

      const allSessions = Object.values(result.sessions).flat();
      return allSessions.some((s) => s.id === sessionId);
    } catch (error) {
      console.error("Error validating session:", error);
      // Assume valid on network error to avoid unnecessary disruption
      return true;
    }
  },
