class StudentManager {
    ...
    async uploadFile() {
        ...
        try {
            ...
            const uploadEndTime = new Date();
            const uploadDuration = ((uploadEndTime - uploadStartTime) / 1000).toFixed(2);
            this.showMessage(`Upload failed after ${uploadDuration}s: ${error.message}`, 'error');
        } finally {
    ...
    displayStudents(students) {
        ...
        if (students.length === 0) {
            ...
            if (isSearching) {
                container.innerHTML = '<div class="no-data">No students match your search criteria.</div>';
            } else {
                container.innerHTML = '<div class="no-data">No students found. Upload a CSV file.</div>';
            }
            return;
        }
                ...
                <tbody>
                    ${students.map(student => `
                        <tr>
                            ...
                            <td class="timestamp">
                                ${this.formatLastChanged(student.seconds_since_update)}
                            </td>
                        </tr>
                    `).join('')}
                </tbody>
    ...
    filterStudents() {
        ...
        const filteredStudents = this.allStudents.filter(student => {
            if (filterBy === 'all') {
                return student.student_id.toLowerCase().includes(searchTerm) ||
